...
namespace Basic_Auth.Services
{
    public class UserServices : IUserService
    {
        ...
        public async Task<User> CreateUserAsync(Userdto user)
        {
            try
            {
                ...
                User userdata = new()
                {
                    ...
                    Role = Enum.TryParse(typeof(UserRole), user.Role, out var parsedRole) ? (UserRole)parsedRole : UserRole.User
                };
        ...
        private  TokenDto DecodeJwtToken(string token)
        {
            ...
            return  new TokenDto{Id=Id != null ? Guid.Parse(Id) : Guid.Empty, Name =Name, Email = Email,Role = Enum.TryParse(typeof(UserRole), Role, out var parsedRole) ? (UserRole)parsedRole : UserRole.User};
